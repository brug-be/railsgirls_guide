...
Have a look how Rails generated a folder structure and a lot of files for you. There are two ways of doing that, in the terminal or with Finder/Explorer. Ask your coach about the details. In your terminal you could simply do:
...
A Rails app contains dynamic code that will be generated by the Ruby programming language. This is why we can't just open a static file anymore in the browser. We need a Rails server that will allow for all the magic to happen.

You start your Rails server as follows. In you terminal type:

{% highlight bash %}
rails server
{% endhighlight %}

Once it's started up, you can go visit your app already under [http://localhost:3000/about.html](http://localhost:3000/about.html). It's not much but the page show you that everything worked fine until now and the server is running.

## The power of Rails: creating a scaffold

One (not so secret) weapon of Rails is it's ability to quickly get you started using scaffolds. Since we're trying to create blog here, let's create a way to create, update and show our blog articles. To do that type the following commands into your terminal:
...
Let's bring the two parts of this guide together. We want to integrate the about page we created in the first part into our Rails app. For that we will use our Rails' generator again and let it create a new controller for us that will have only one page. To do that open the terminal again and type the following line:
...
Our about page contains a couple of pieces of information that we can use to showcase why using Rails and
